...
int main(int argc, char* argv[]) {
    ...
    for (int iEvent = 0; iEvent < nEvent; ++iEvent) {
        ...
        for (int i = 0; i < pythia.event.size(); ++i) {
            ...
            particleTemp.set_user_info(new MyInfo(pdg_id, i, charge)); //Needs to be implemented
        ...
        for(int jetid = 0; jetid < selectedJets.size(); jetid++){
            ...
            for (unsigned j = 0; j < sortedconstituents.size(); j++){
                ...
                if (abs(sortedconstituents[j].user_info<MyInfo>().pdg_id()) == triggerId){ //!here it gets stored
