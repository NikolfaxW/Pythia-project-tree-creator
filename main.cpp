...
void safeFill(TTree *&tree) {
    std::lock_guard<std::mutex> lock(std::mutex);
    tree->Fill();
}

void mainSec(int numThreads, TTree *&T, Float_t &D_0_Pt, Float_t &Jet_Pt, Float_t &rapidity, Float_t &hasD_0,
             Float_t &EVI, Float_t &z_val) {
    ...
    for (int iEvent = 0;
         iEvent < pythia.mode("Main:numberOfEvents") / numThreads; ++iEvent) { //loop over needed number of events
...
//    unsigned int numThreads = 2;
    ...
    TFile *file = new TFile("../results/Jet_tree3.root", "RECREATE"); //create a file to store the tree as an output
    ...
    for (int i = 0; i < numThreads; i++) {
        alocatedThreads[i] = new std::thread(mainSec, numThreads, std::ref(T), std::ref(D_0_Pt), std::ref(Jet_Pt), std::ref(rapidity), std::ref(hasD_0), std::ref(EVI),
                                             std::ref(z_val));
    }
    for (int i = 0; i < numThreads; i++) {
